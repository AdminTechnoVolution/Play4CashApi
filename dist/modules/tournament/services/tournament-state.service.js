...
let TournamentStateService = class TournamentStateService {
    ...
    async getBracket(tournamentId) {
        ...
        const participants = await this.participantModel
            .find({
            tournament_id: oid,
            status: { $ne: tournament_constants_1.TournamentParticipantStatus.REFUNDED },
        })
            .sort({ seed: 1 })
            .lean();
        ...
        for (const p of participants) {
            ...
            if (!gn)
                continue;
            if (!rosterByGroup.has(gn))
                rosterByGroup.set(gn, []);
        ...
        const mapParticipantList = (groupNumber) => rosterByGroup.get(groupNumber) ?? [];
