...
let ChessGateway = ChessGateway_1 = class ChessGateway {
    ...
    constructor(chessGameModel, roomModel, userModel, config, roomsGateway, redis, i18n, grace, turnDeadlines, webPush, tournamentMatchService) {
    ...
    onModuleInit() {
        ...
        this.turnDeadlines.registerHandler('chess', (_playerId, roomId) => this.executeTurnTimeout(roomId));
    }
    ...
    async handleMove(client, payload) {
        ...
        for (const s of sockets) {
            ...
            s.emit('chess', {
                success: true,
                data: await (0, game_state_version_util_1.enrichGamePayload)(this.redis, 'chess', room_id, sData, {
    ...
    startTimer(socket, room_id, seconds) {
        ...
        if (playerId)
            void this.turnDeadlines.schedule('chess', room_id, playerId, seconds);
        const t = setTimeout(() => void this.executeTurnTimeout(room_id, socket.id), seconds * 1000);
        turnTimers.set(socket.id, t);
    }
    async executeTurnTimeout(room_id, socketId) {
        if (socketId)
            clearTimer(socketId);
