...
let ConnectFourGateway = ConnectFourGateway_1 = class ConnectFourGateway {
    ...
    constructor(gameModel, roomModel, userModel, config, roomsGateway, redis, i18n, grace, turnDeadlines, tournamentMatchService) {
    ...
    onModuleInit() {
        ...
        this.turnDeadlines.registerHandler('connect-four', (_playerId, roomId) => this.executeConnectFourTurnTimeout(roomId));
    }
    afterInit(server) {
    ...
    async handleDropDisc(client, payload) {
        ...
        for (const s of sockets) {
            try {
                ...
                s.emit(EVENT, {
                    success: true,
                    data: await (0, game_state_version_util_1.enrichGamePayload)(this.redis, 'connect-four', room_id, {
                        ...sState,
                        lastMove,
                        gameEnded: finished,
    ...
    startTimer(socket, room_id, seconds) {
        ...
        if (playerId)
            void this.turnDeadlines.schedule('connect-four', room_id, playerId, seconds);
        const t = setTimeout(() => void this.executeConnectFourTurnTimeout(room_id), seconds * 1000);
        turnTimers.set(socket.id, t);
    }
    async executeConnectFourTurnTimeout(room_id) {
