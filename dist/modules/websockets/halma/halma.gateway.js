...
const game_state_version_util_1 = require("../../../common/utils/game-state-version.util");
const web_push_service_1 = require("../../../common/web-push/web-push.service");
...
let HalmaGateway = HalmaGateway_1 = class HalmaGateway {
    ...
    constructor(halmaModel, roomModel, userModel, config, roomsGateway, redis, i18n, grace, turnDeadlines, webPush, tournamentMatchService) {
    ...
    onModuleInit() {
        ...
        this.turnDeadlines.registerHandler('halma', (_playerId, roomId) => this.executeHalmaTurnTimeout(roomId));
    }
    ...
    async handleMove(client, payload) {
        ...
        for (const s of sockets) {
            ...
            s.emit('halma', {
                success: true,
                data: await (0, game_state_version_util_1.enrichGamePayload)(this.redis, 'halma', room_id, sData, {
    ...
    async handleEndTurn(client, payload) {
        ...
        if (opponent) {
            ...
            const opponentId = opponent.data?.player_id;
            if (opponentId)
                this.webPush.notifyYourTurn(opponentId, 'halma', room_id);
        }
        const b = game.board;
    ...
    startTimer(socket, room_id, seconds) {
        ...
        const playerId = socket.data?.player_id || '';
        if (playerId)
            void this.turnDeadlines.schedule('halma', room_id, playerId, seconds);
        const t = setTimeout(() => void this.executeHalmaTurnTimeout(room_id), seconds * 1000);
        turnTimers.set(socket.id, t);
    }
    async executeHalmaTurnTimeout(room_id) {
