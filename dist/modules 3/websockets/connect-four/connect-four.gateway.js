...
let ConnectFourGateway = ConnectFourGateway_1 = class ConnectFourGateway {
    ...
    constructor(gameModel, roomModel, userModel, config, roomsGateway, redis, i18n, grace) {
    ...
    async executeForfeit(room_id, player_id) {
        const room = await this.roomModel.findOne({
            _id: new mongoose_2.Types.ObjectId(room_id),
            status: 'started',
        });
        if (!room)
            return;
    ...
    async handleJoin(client, payload) {
        ...
        if (room.status === 'started' && game && !client.data.isSpectator) {
            ...
            if (isMyTurn) {
                const timerSeconds = state.turnTimerSeconds;
